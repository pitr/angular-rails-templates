...
describe "angular-rails4-templates.js integration" do
  let(:config) { Dummy::Application.config.angular_templates }

  it "serves angular-rails4-templates.js on the pipeline" do
    visit '/assets/angular-rails4-templates.js'
    page.source.must_include %Q{angular.module("#{config.module_name}", [])}
  end

  it "includes a comment with the gem name and version" do
    visit '/assets/angular-rails4-templates.js'
    page.source.must_include %Q{// Angular Rails Templates #{AngularRails4Templates::VERSION}}
  end
